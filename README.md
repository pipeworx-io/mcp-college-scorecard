...
`tools/list` at `https://gateway.pipeworx.io/college-scorecard/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/search_schools \
  -H 'Content-Type: application/json' \
  -d '{"query":"Stanford University","state":"CA"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/search_schools`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "college-scorecard": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-college-scorecard"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-college-scorecard
```
